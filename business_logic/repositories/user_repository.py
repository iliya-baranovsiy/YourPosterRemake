...
class UserRepository:
    ...
    async def update(self, user: User, start_date=None):
        await self.orm.update_user_data(tg_id=user.tg_id, balance=user.balance,
                                        payment_plan=user.subscription.payment_plan, automatic_buy=user.automatic_buy,
                                        pending_plan=user.subscription.pending_plan.STANDART,
                                        priority=user.subscription.priority, activate_date=start_date,
