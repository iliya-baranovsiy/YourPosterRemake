...
def get_amount_stars_kb():
    kb = [
        [InlineKeyboardButton(text="130 ⭐ ≈ 3.10 $", callback_data=StarsDepositCb(amount=130).pack())],
        [InlineKeyboardButton(text="260 ⭐ ≈ 6.20 $", callback_data=StarsDepositCb(amount=260).pack())],
        [InlineKeyboardButton(text="520 ⭐ ≈ 12.40 $", callback_data=StarsDepositCb(amount=520).pack())],
...
def get_pay_kb(amount: int):
    ...
    kb.append([InlineKeyboardButton(text="◀️ Назад", callback_data="stars_deposit")])
    return InlineKeyboardMarkup(inline_keyboard=kb)
